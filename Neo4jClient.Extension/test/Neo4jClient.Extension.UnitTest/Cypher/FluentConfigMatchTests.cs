...
OPTIONAL MATCH (ha:Address)", text);
        ...
        public void OptionalMatchRelationship()
        {
            ...
            var q = GetFluentQuery()
                .MatchEntity(person)
                .OptionalMatchRelationship(homeAddressRelationship,
                    MatchRelationshipOptions.Create().WithNoProperties());
            var text = q.GetFormattedDebugText();
            Console.WriteLine(text);
...
OPTIONAL MATCH (person)-[personaddress:HOME_ADDRESS]->(address)", text);
        ...
        {
            var addressRelationship =
                new HomeAddressRelationship(DateTimeOffset.Parse("2015-08-05 12:00"), "agent", "homeAddress");
            var q = GetFluentQuery()
                .MatchRelationship(addressRelationship);
            var text = q.GetFormattedDebugText();
            ...
            Assert.AreEqual(
                @"MATCH (agent)-[agenthomeAddress:HOME_ADDRESS {dateEffective:{agenthomeAddressMatchKey}.dateEffective}]->(homeAddress)",
                text);
        }
    }
}